...
const { optics, waffle } = require('hardhat');
const { provider } = waffle;
const { expect } = require('chai');

import { HexString } from '../lib/types';
const testUtils = require('../lib/utils');
import { localConfig, remoteConfig } from '../../../chainConfig';
import {
  devDeployNewReplica,
...
describe('Replica', async () => {
  let localDeploy: chain.Deploy, remoteDeploy: chain.Deploy;
  let replica: any,
    signer: ethers.Signer,
    fakeSigner: ethers.Signer,
    updater: any,
    fakeUpdater: ethers.Contract,
    initializeArgs: any;

  const enqueueValidUpdate = async (newRoot: HexString) => {
    let oldRoot;
  ...
  beforeEach(async () => {
    localDeploy = chain.freshDeploy(localConfig);
    remoteDeploy = chain.freshDeploy(remoteConfig);

    await deployUpdaterManager(localDeploy);
    await deployUpgradeBeaconController(localDeploy);

    // await deployUpdaterManager(remoteDeploy);
    // await deployUpgradeBeaconController(remoteDeploy);
    ...
    await devDeployNewReplica(localDeploy, remoteDeploy, true);
    ...
    replica = localDeploy.contracts.replicas[remoteDeploy.chain.domain];
  });
  ...
  it('Halts on fail', async () => {
    console.log(replica.proxy);
    await replica.proxy.setFailed();
    const state = await replica.proxy.state();
    console.log(state);
    // expect(state).to.equal(optics.State.FAILED);
